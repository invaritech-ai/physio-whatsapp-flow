...
def test_admin_reports_payroll_counts_new_durations(client, db_session: Session):
    ...
    # No payout configured for these durations yet → zero estimated pay.
    assert item["estimated_payable_cents"] == 0
...
def test_admin_reports_payroll_uses_configured_payouts(client, db_session: Session):
    ...
    _set_payout(db_session, therapist.id, 15, 20000)
    _set_payout(db_session, therapist.id, 60, 90000)
    for minutes in (15, 15, 60):
        _create_session(
            db_session,
            client_id=client_row.id,
            therapist_id=therapist.id,
    ...
    # 2 x 15min @ 20000 + 1 x 60min @ 90000 = 130000
    assert item["estimated_payable_cents"] == 130000
...
def test_admin_reports_payroll_detail(client, db_session: Session):
    ...
    # 20000 (15min) + 90000 (60min) = 110000
    assert data["total_pay_cents"] == 110000
    assert len(data["sessions"]) == 2
    payouts = sorted(s["payout_cents"] for s in data["sessions"])
    assert payouts == [20000, 90000]
