...
def test_suspend_therapist_user_also_deactivates_therapist_profile(client, db_session: Session):
    actor_admin = User(
        neon_auth_sub="cascade-actor-admin-sub",
        email="cascade-actor-admin@test.com",
        display_name="Cascade Actor Admin",
        role="admin",
        is_active=True,
    )
    target_user = User(
        neon_auth_sub="cascade-target-sub",
        email="cascade-target@test.com",
    ...
    therapist = Therapist(
        ...
        is_active=True,
        calendly_user_uri="https://api.calendly.com/users/cascade-target",
    )
    ...
    # Reactivating restores bookability for onboarded therapists.
    with patch("app.core.auth._verify_neon_token") as mock_verify:
        mock_verify.return_value = {
            "sub": actor_admin.neon_auth_sub,
            "email": actor_admin.email,
...
def test_reactivate_user_does_not_activate_unonboarded_therapist_profile(client, db_session: Session):
    actor_admin = User(
        neon_auth_sub="unonboarded-actor-admin-sub",
        email="unonboarded-actor-admin@test.com",
        display_name="Unonboarded Actor Admin",
        role="admin",
        is_active=True,
    )
    target_user = User(
        neon_auth_sub="unonboarded-target-sub",
        email="unonboarded-target@test.com",
        display_name="Unonboarded Target",
        role="therapist",
        is_active=False,
    )
    db_session.add(actor_admin)
    ...
    # No calendly_user_uri — onboarding never completed.
    therapist = Therapist(
        user_id=target_user.id,
        display_name=target_user.display_name,
        is_active=False,
    ...
    db_session.refresh(therapist)
    assert target_user.is_active is True
    assert therapist.is_active is False  # stays gated behind onboarding
